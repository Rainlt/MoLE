...
class CHAIR(object):
    def __init__(self, imids, coco_path):
        self.llama_tokenizer = AutoTokenizer.from_pretrained("/sda/liangtian/model/llava-v1.5-7b/", use_fast=False)
        self.imid_to_objects = {imid: [] for imid in imids}

        self.coco_path = coco_path

        # read in synonyms
        synonyms = open("eval/chair_metrics/data/synonyms.txt").readlines()
    ...
    def compute_chair(self, cap_file):
        ...
        def resize_caption_to_max_length(caption, max_length):
            #first use tokenizer to tokenize the caption
            caption = self.llama_tokenizer.tokenize(caption)
            #then resize the caption to max_length
            if len(caption) > max_length:
                caption = caption[:max_length]
            #then convert the caption back to string
            caption = self.llama_tokenizer.convert_tokens_to_string(caption)

            return caption
        
        for i, cap_eval in enumerate(caps):
            cap = cap_eval["caption"]
            cap = resize_caption_to_max_length(cap, 128)
