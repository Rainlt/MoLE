...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        #hidden_states_attn = hidden_states#addddd
        hidden_states = self.mlp(hidden_states)
        hidden_states = residual + hidden_states
        #residual+hidden_states，还未layer norm
        hidden_states_attn = hidden_states#算了，还是用hidden_states。相当于没改过
        ...
        output_attn_hidden = True
        if output_attn_hidden:
            outputs += (hidden_states_attn,)
        return outputs
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        hidden_states_attn = inputs_embeds#addddd
        ...
        for idx, decoder_layer in enumerate(self.layers):
            if output_hidden_states:
                all_hidden_states += (hidden_states_attn,)#(hidden_states,)
            ...
            hidden_states = layer_outputs[0]#这里不应该改，因为是每一层往后的输入，我们想改的是输出，因此直接后面保存hidden_states_attn就好
            hidden_states_attn = layer_outputs[-1]
        ...
        hidden_states_attn = self.norm(hidden_states_attn)#adddddd
        # add hidden states from the last decoder layer
        if output_hidden_states:
            all_hidden_states += (hidden_states_attn,)#(hidden_states,)#这里加上最后一层的hidden states
