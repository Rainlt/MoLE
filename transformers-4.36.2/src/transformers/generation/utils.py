...
class GenerationMixin:
    ...
    def generate(
        ...
        halc_assistant=None,
        # MoLE's kwargs
        mole_decoding: Optional[bool] = None,
        tmp: Optional[float] = 1.0,
        topN_layers: Optional[int] = 0,
        final_layers: Optional[int] = 0,
        w_exp: Optional[float] = 1.0,
        # OPERA's kwargs
        ...
        **kwargs,
        ...
        # 10. go into different generation modes
        ...
        elif generation_mode == GenerationMode.GREEDY_SEARCH and mole_decoding:
            print("\033[41m!!!!! MoLE Decoding !!!!!!\033[0m")
            if generation_config.num_return_sequences > 1:
                raise ValueError(
                    f"num_return_sequences has to be 1, but is {generation_config.num_return_sequences} when doing"
                    " greedy search."
            ...
            # 11. run greedy search
            # print("\033[41m!!!!! DoLA-Greedy Decoding !!!!!!\033[0m")
            return self.mole_greedy_decode(
                input_ids,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
                pad_token_id=generation_config.pad_token_id,
        ...
        elif generation_mode == GenerationMode.BEAM_SEARCH and mole_decoding:
            print("\033[41m!!!!! MOLE-BEAM Decoding !!!!!!\033[0m")
            # 11. run dola beam search
            if generation_config.num_return_sequences > generation_config.num_beams:
                raise ValueError("`num_return_sequences` has to be smaller or equal to `num_beams`.")
    ...
    def relative_top_filter(
        ...
        scores_normalized = scores.log_softmax(dim=-1)#先做一个log softmax
        sorted_logits, sorted_indices = torch.sort(scores_normalized, descending=True)#排序
        min_thresh = sorted_logits[..., min_tokens_to_keep - 1]#
        probs_max = torch.max(scores_normalized, dim=-1).values#最大值
        probs_thresh = probs_max + np.log(relative_top)#最大值+log(0.1)，其实就是log(最大值*0.1)
        probs_thresh = torch.min(min_thresh, probs_thresh)#和序列的最小值相比，取更小的那个
        probs_thresh = probs_thresh.unsqueeze(-1)
        scores_normalized[scores_normalized < probs_thresh] = filter_value#小yu阈值的值都置为-inf？
        return scores_normalized


    def mole_greedy_decode(
        self,
        input_ids: torch.LongTensor,
        mature_layer: int,
        base_layer: Optional[int] = None,
        ...
        **model_kwargs,
        ...
        all_layer_logits_matrix = []
        all_layer_tokens_matrix = []
        all_layer_real_tokens_matrix = []
        all_layer_real_logits_matrix = []
        all_layer_log_logits_matrix = []
        all_layer_log_tokens_matrix = []
        first_record = True
        start_pos = 0
        count = 0
        t = torch.tensor(0.0, device = input_ids.device)
        while True:
            if synced_gpus:
                # Under synced_gpus the `forward` call must continue until all gpus complete their sequence.
                # The following logic allows an early break if all peers finished generating their sequence
            ...
            )#outputs['attentions']:(32,)*[1, 32, n, n]。从第二个token开始，变为(32,)*[1, 32, 1, n]
            if first_record:
                start_pos = outputs['attentions'][0].shape[-1]
                first_record = False
            #{'image_start': 34, 'image_end': 609, 'response_start': 623} img:[34:610] num_img_token for llava:576
            #pdb.set_trace()
            before_attention = torch.tensor([outputs['attentions'][i][0,-1,-1,:start_pos-1].sum() for i in range(32)])#最后一个head在每层对前面的attention #[32]
            attn_matrix.append(before_attention)#([attn.item() for attn in before_attention])
            max_attn_index = max(range(len(before_attention)), key=before_attention.__getitem__)#add 16 when only count half layers
            
            def calculate_jsd(logits_mature, logits_premature):
                # 1. Calculate the softmax values for mature_layer and all premature_layers
                softmax_mature_layer = F.softmax(
                    logits_mature, dim=-1
                )  # shape: (batch_size, num_features)
                softmax_premature_layers = F.softmax(
                    logits_premature, dim=-1
                )  # shape: (num_premature_layers, batch_size, num_features)

                # 2. Calculate M, the average distribution
                M = 0.5 * (
                    softmax_mature_layer[None, :, :] + softmax_premature_layers
                )  # shape: (num_premature_layers, batch_size, num_features)

                # 3. Calculate log-softmax for the KL divergence
                ...
                return js_divs
            def calculate_jsd_all(dict_outputs, mature_layer, candidate_premature_layers):
            ...
            if base_layer is not None:
                ...
                if relative_top > 0.0:
                    final_logits = self.relative_top_filter(final_logits, relative_top)#这里是有log softmax的，所以后面对base也要log softmax
                ...
                # 1. Stacking all premature_layers into a new dimension
                stacked_all_layers = torch.stack(
                    [dict_outputs[i][:, -1, :] for i in candidate_premature_layers+[mature_layer]], dim=0
                )#[33,1,32000]
                # 2. Select the topN tokens and logits of the mature layer
                topN = topN_layers
                topN_logits_mature, topN_indices_mature = torch.topk(stacked_all_layers[-1,:,:], topN, dim=-1)#[1, 5]; [1, 5]
                # 3. Select the remaining tokens and logits of the mature layer
                total_indices = torch.arange(stacked_all_layers.size(-1), device=stacked_all_layers.device)#[32000]
                remaining_indices = total_indices[~torch.isin(total_indices, topN_indices_mature.squeeze(0))]#[31995]
                remaining_logits_mature = stacked_all_layers[-1,:,remaining_indices]#[1,31995] 最后一层的，除了topN之外的剩下的token的logits，注意顺序

                # 4. Select the same tokens' logits of topN_indices_mature and remain_indices of the premature layers
                topN_logits_premature = stacked_all_layers[:-1,:,topN_indices_mature.squeeze(0)]#[32, 1, 5]，除了最后一层之外的，32个premature layer的topN token的logits
                remaining_logits_premature = stacked_all_layers[:-1,:,remaining_indices]#[32, 1, 31995]，除了最后一层之外的，32个premature layer的剩下的token的logits

                js_divs_topN = calculate_jsd(topN_logits_mature, topN_logits_premature)[-3:]*10000#仅在最后三层中选
                js_divs_remaining = calculate_jsd(remaining_logits_mature, remaining_logits_premature)[-3:]*10000#同上
                js_divs = calculate_jsd_all(dict_outputs, mature_layer, candidate_premature_layers)
                #如果js_divs_topN的最大值和js_divs_remaining的最小值为同一个，那么就是这个
                topN_layer = int(js_divs_topN.argmax().cpu().item())
                remain_layer = int(js_divs_remaining.argmin().cpu().item())
                if(topN_layer==remain_layer):
                    expert_layer = 29+topN_layer
                    expert_weight = 1
                else:
                    expert_layer = 0
                    expert_weight = 0
                ...
                #premature_layer_2 = candidate_premature_layers[indices[1]]
                ...
                all_layer_logits = []
                all_layer_tokens = []
                all_layer_log_logits = []
                all_layer_log_tokens = []
                all_layer_real_logits = []
                all_layer_real_tokens = []
                all_layer = candidate_premature_layers + [mature_layer]
                #将all_layer倒置
                all_layer = all_layer[::-1]
                for layer in all_layer:#candidate_premature_layers + [mature_layer]:
                    layer_logits = dict_outputs[layer][:, -1, :]
                    log_layer_logits = layer_logits.log_softmax(dim=-1)
                    if(layer==mature_layer):
                        selected_logits, selected_tokens = torch.topk(layer_logits, k=20, dim=-1)
                        log_selected_logits, log_selected_tokens = torch.topk(log_layer_logits, k=20, dim=-1)
                    else:
                        selected_logits = layer_logits[:,selected_tokens.squeeze(0)]
                        log_selected_logits = log_layer_logits[:,selected_tokens.squeeze(0)]
                    #最后一层是top20的logits，但是其他层要根据最后一层的token来拿到对应的logits

                    all_layer_logits.append(selected_logits.cpu())
                    all_layer_tokens.append(selected_tokens.cpu())

                    layer_real_logits, layer_real_tokens = torch.topk(layer_logits, k=20, dim=-1)
                    all_layer_real_tokens.append(layer_real_tokens.cpu())
                    all_layer_real_logits.append(layer_real_logits.cpu())
                    all_layer_log_logits.append(log_selected_logits.cpu())
                    all_layer_log_tokens.append(log_selected_tokens.cpu())  
                # "▁clock": 12006
                # "▁sur": 1190
                all_layer_logits_matrix.append(torch.cat(all_layer_logits[::-1], dim=0).numpy().tolist())#用[::-1]将layer顺序还原回0，1，2。。。
                all_layer_tokens_matrix.append(torch.cat(all_layer_tokens[::-1], dim=0).numpy().tolist())
                all_layer_real_logits_matrix.append(torch.cat(all_layer_real_logits[::-1], dim=0).numpy().tolist())
                all_layer_real_tokens_matrix.append(torch.cat(all_layer_real_tokens[::-1], dim=0).numpy().tolist())
                all_layer_log_logits_matrix.append(torch.cat(all_layer_log_logits[::-1], dim=0).numpy().tolist())
                all_layer_log_tokens_matrix.append(torch.cat(all_layer_log_tokens[::-1], dim=0).numpy().tolist())
                
                base_logits = dict_outputs[premature_layer][:, -1, :]
                
                expert_logits = dict_outputs[expert_layer][:,-1,:]
                base_logits_attn = dict_outputs[max_attn_index][:, -1, :]
                attn_layer_weights = 1 - torch.exp(-t/tmp)
                final_logits = dict_outputs[mature_layer][:, -1, :]
                
                if(w_exp==0 and tmp==1000):#greedy
                    logits = final_logits
                elif(w_exp==0):#no exp
                    logits = final_logits  +  attn_layer_weights*base_logits_attn
                elif(tmp==1000):#no attn
                    logits = final_logits  + w_exp*expert_weight*expert_logits
                else:
                    logits = torch.topk(torch.stack([final_logits, expert_weight * expert_logits, base_logits_attn]), 2, dim=0).values.sum(dim=0)
                next_token_logits = logits
        ...
        info_dict["all_layer_logits"] = all_layer_logits_matrix
        info_dict["all_layer_tokens"] = all_layer_tokens_matrix
        info_dict["all_layer_real_logits"] = all_layer_real_logits_matrix
        info_dict["all_layer_real_tokens"] = all_layer_real_tokens_matrix
        info_dict["all_layer_log_logits"] = all_layer_log_logits_matrix
        info_dict["all_layer_log_tokens"] = all_layer_log_tokens_matrix
        info_dict["premature_layer_list"] = premature_layer_list
        ...
        **model_kwargs,
        ...
        while True:
            ...
            for bs in range(beam_size):
                ...
                beam_last_word_flag[bs] = self.halc_assistant.check_word_complete(beam_next_tokens[bs][:, None])#判断生成的token是否是单词的最后一个token
                
                if beam_last_word_flag[bs] == False:#如果不是1，就不管
                    ...
                    if len(beam_last_tokens[bs]) == 0:
                        ...
                        beam_current_word[bs] = self.halc_assistant.get_last_word(beam_last_tokens[bs])#将上个t生成的token解码 
                        # print("beam_last_tokens: ", beam_last_tokens[bs])

                        # print("CURRENT WORD: ", beam_current_word[bs])
                        if self.halc_assistant.check_word_complete([beam_last_tokens[bs]]) == False:#上个t生成的单词是否是最后一个，如果不是
                            ...
                            while True:
                                ...
                                concat_words = torch.cat([beam_intermediate_token_lists[bs][:, -t:], torch.tensor([beam_last_tokens[bs]]).to(beam_input_ids[bs].device)], dim=-1)#一直concat之前的token，直到完整为止
    ...
    def evolve_vcd_sampling(
        ...
        **model_kwargs,
        ...
        #dola
        early_exit_layers = candidate_premature_layers + [32]
        # init values
        logits_processor = logits_processor if logits_processor is not None else LogitsProcessorList()
        stopping_criteria = stopping_criteria if stopping_criteria is not None else StoppingCriteriaList()
        if max_length is not None:
        ...
        while True:
            ...
            next_token_logits = outputs.logits[:, -1, :]
            #next_token_logits_mid = dict_outputs[selected_layer][:,-1,:]
            ...
            if use_cd:
                ...
                # print("self", self)
                dict_outputs_cd, outputs_cd = self(
                    **model_inputs_cd,
                    return_dict=True,
                ...
                #next_token_logits_cd = dict_outputs_cd[selected_layer][:,-1,:]
                ...
                cutoff = torch.log(torch.tensor(cd_beta)) + next_token_logits.max(dim=-1, keepdim=True).values#这里因为1next_token_logits是log后1的结果，所以相当于相乘？
                
                
                diffs = (1+cd_alpha)*next_token_logits - cd_alpha*next_token_logits_cd
                #diffs = next_token_logits + cd_alpha*(next_token_logits_mid-next_token_logits_cd)#addd
                cd_logits = diffs.masked_fill(next_token_logits < cutoff, -float("inf"))#对比过后，不够大的就直接去掉
    ...
    def mole_beam_search(
        ...
        **model_kwargs,
        ...
        def calculate_jsd(logits_mature, logits_premature):#logits_mature:[3,5]
                # 1. Calculate the softmax values for mature_layer and all premature_layers
                softmax_mature_layer = F.softmax(
                    logits_mature, dim=-1
                )  # shape: (batch_size, num_features)
        ...
        while True:
            ...
            # forward pass to get next token
            #pdb.set_trace()
            dict_outputs, outputs = self(#用attention来选择compare layer，而不是用JS divergence来选择compare layer是否会更好？
                **model_inputs,
                return_dict=True,
                output_attentions=output_attentions,
                output_hidden_states=output_hidden_states,
            ...
            if first_record:
                start_pos = outputs['attentions'][0].shape[-1]
                first_record = False
            #before_attention = [outputs['attentions'][i][0,-1,-1,34:610].sum() for i in range(32)]
            before_attention = torch.tensor([outputs['attentions'][i][0,-1,-1,:start_pos-1].sum() for i in range(32)])
            max_attn_index = max(range(len(before_attention)), key=before_attention.__getitem__)
            if base_layer is not None:
                base_logits = dict_outputs[base_layer][:, -1, :]
                final_logits = dict_outputs[mature_layer][:, -1, :]
                ...
                logits = final_logits - base_logits
                next_token_logits = logits
            else:#因为num_beam和batch size乘在一起的[batch*num_beam, ...]，所以这里可以先不用管num_beam
                #split topN logits and calculate jsd
                # 1. Stacking all premature_layers into a new dimension
                stacked_all_layers = torch.stack(
                    [dict_outputs[i][:, -1, :] for i in candidate_premature_layers+[mature_layer]], dim=0
                )#[33,3,32000]
                ...
                topN_logits_mature, topN_indices_mature = torch.topk(stacked_all_layers[-1,:,:], topN, dim=-1)#[3,5]; [3,5] #1111111111111
                
                # 3. Select the remaining tokens and logits of the mature layer
                total_indices = torch.arange(stacked_all_layers.size(-1), device=stacked_all_layers.device).repeat(stacked_all_layers.size(1), 1)#[3, 32000]
                # 扩展 topN_indices_mature 以便与 total_indices 具有相同的形状
                topN_indices_expanded = topN_indices_mature.unsqueeze(-1)  # [3, 5, 1]
                total_indices_expanded = total_indices.unsqueeze(1)  # [3, 1, 32000]

                # 生成布尔掩码，标记出与 topN_indices_mature 相同的元素
                mask = (total_indices_expanded == topN_indices_expanded).any(dim=1)  # [3, 32000]

                # 过滤出与 topN_indices_mature 不同的元素
                remaining_indices = total_indices[~mask].reshape(stacked_all_layers.size(1), -1)  # [3, 31995]
                #remaining_indices_mature = remaining_indices.unsqueeze(0).expand(1, -1, -1)#不做expand
                remaining_indices_premature = remaining_indices.unsqueeze(0).expand(32, -1, -1)
                #expanded_remaining_index = remaining_indices.unsqueeze(0).expand(stacked_all_layers.size(0), -1, -1)
                remaining_logits_mature = torch.gather(stacked_all_layers[-1,:,:], 1, remaining_indices)# [3, 31995]dim是要取的那个维度，一般都是最后一维
                
                remaining_logits_premature = torch.gather(stacked_all_layers[:-1,:,:], 2, remaining_indices_premature)#[32, 3, 31995]
                
                #将topN_indices_mature的shape从[3,5]扩展到[32, 3, 5]
                topN_indices_premature = topN_indices_mature.expand(32, -1, -1)
                topN_logits_premature = torch.gather(stacked_all_layers[:-1,:,:], 2, topN_indices_premature)#[32, 3, 5]
                
                js_divs_topN = calculate_jsd(topN_logits_mature, topN_logits_premature)[-final_layers:]*10000#仅在最后三层中选
                js_divs_remaining = calculate_jsd(remaining_logits_mature, remaining_logits_premature)[-final_layers:]*10000#同上
                # 1. Stacking all premature_layers into a new dimension
                stacked_premature_layers = torch.stack(
                    [dict_outputs[i][:, -1, :] for i in candidate_premature_layers], dim=0
                )
                logits_mature = dict_outputs[mature_layer][:, -1, :]
                logits_premature = stacked_premature_layers
                js_divs = calculate_jsd(logits_mature, logits_premature)
                ...
                if(w_exp==0 and tmp==1000):#beam
                    ...
                    logits = final_logits  + w_exp*expert_weight*expert_logits + attn_layer_weights*base_logits_attn 
