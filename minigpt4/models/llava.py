...
class LLaVa(BaseModel):
    ...
    def generate(
        ...
        '''
        output_tokens = [self.llama_tokenizer.convert_ids_to_tokens(id) for id in output_ids[:, input_token_len:].cpu()][0]  # [n]
        info_dict['jsd_matrix'] = info_dict['jsd_matrix'].cpu().T.numpy().tolist()
        #info_dict['attn_matrix'] = info_dict['attn_matrix'].cpu().T.numpy().tolist()
        #info_dict['jsd_matrix_topN'] = info_dict['jsd_matrix_topN'].cpu().T.numpy().tolist()
        #info_dict['jsd_matrix_remain'] = info_dict['jsd_matrix_remain'].cpu().T.numpy().tolist()
        info_dict['output_tokens'] = output_tokens
        
        output_addition = info_dict
        '''
        ...
        return output_text#, output_addition
    ...
    def from_config(cls, cfg):
        vision_tower = cfg.get("vit_model", r'openai/clip-vit-large-patch14-336')
