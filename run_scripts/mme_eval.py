...
for idx in tqdm(range(iterations)):
    ...
    with torch.inference_mode():
        with torch.no_grad():
            out = model.generate(
                ...
                # HALC
                halc_assistant=None,#halc_assistant_helper,
                # OPERA
                key_position=None,
    ...
    generated_captions_path = os.path.join(
        base_dir,
        f"{model_name}_{decoding_strategy}_{detector_type}_box_{box_threshold}_beams_{num_beams}_k_{k_candidate_num}_{dataset_name}_expand_ratio_{expand_ratio}_seed_{seed}_max_tokens_{max_new_tokens}_samples_{num_samples}_skip_{skip_num}_wexp_{args.w_exp}_generated_captions.json",
    )
    ...
    method_dir = os.path.join(base_dir, f"{decoding_strategy}")
    #判断method_dir是否存在
    if not os.path.exists(method_dir):
        os.makedirs(method_dir)
    txt_path = os.path.join(method_dir, f"{dataset_name}.txt")
