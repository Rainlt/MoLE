...
MODEL_EVAL_CONFIG_PATH = {
    ...
    #"minigpt4_llama2": "eval_configs/minigpt4_llama2_eval.yaml",
...
parser.add_argument(
    ...
    type=float,
    default=0.2,
    help="weight param for expert layer.",
)
...
for idx, img_id in tqdm(enumerate(range(len(img_files))), total=len(img_files)):
    ...
    image_path = args.data_path + img_file#args.data_path: './coco2014/val2014/', img_file: 'COCO_val2014_000000230008.jpg'
    #for llava_bench
    #image_path = './llava-bench/images/012.jpg'
    ...
    with torch.inference_mode():
        with torch.no_grad():
            ...
            out = model.generate(
                {"image": norm(image), "prompt":qu, "img_path": image_path},#qu: 'USER: <ImageHere> Please describe this image in detail. ASSISTANT:'
                ...
                #MoLE
                mole_decoding=mole_decoding,#注意在各模型的generate中加上这些参数
                tmp=args.tmp,
    ...
    generated_captions_path = os.path.join(
        base_dir,
        f"{model_name}_{decoding_strategy}_{detector_type}_box_{box_threshold}_beams_{num_beams}_k_{k_candidate_num}_{dataset_name}_expand_ratio_{expand_ratio}_seed_{seed}_max_tokens_{max_new_tokens}_samples_{num_samples}_skip_{skip_num}_tmp_{args.tmp}_topN_{args.topN_layers}_w_{args.w_exp}_final_{args.final_layers}_generated_captions_tuning_{args.add}.json",
    )
...
#with open('token_list_caption_128_addID.json','w') as f:   
...
generated_captions_path = os.path.join(
        base_dir,
        f"{model_name}_{decoding_strategy}_{detector_type}_box_{box_threshold}_beams_{num_beams}_k_{k_candidate_num}_{dataset_name}_expand_ratio_{expand_ratio}_seed_{seed}_max_tokens_{max_new_tokens}_samples_{num_samples}_skip_{skip_num}_tmp_{args.tmp}_topN_{args.topN_layers}_w_{args.w_exp}_generated_captions_tuning_{args.add}.json",
    )
'''
loaded_json = []
...
img_to_eval_dict = {}
# to save memory, load 100 captions at a time 可能会报内存溢出错误，此时可以降低每次的caption数量或者增加给java分配的内存：export _JAVA_OPTIONS='-Xmx100g'就是分配100g内存
...
formulated_output_path = os.path.join(
    base_dir,
    f"{model_name}_{decoding_strategy}_{detector_type}_beams_{num_beams}_k_{k_candidate_num}_{dataset_name}_expand_ratio_{expand_ratio}_seed_{seed}_max_tokens_{max_new_tokens}_samples_{num_samples}_tmp_{args.tmp}_topN_{args.topN_layers}_w_{args.w_exp}_final_{args.final_layers}_chair_tuning_{args.add}.json",
)
